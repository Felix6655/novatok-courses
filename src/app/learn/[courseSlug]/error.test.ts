...
import LearnCourseError from "@/app/learn/[courseSlug]/error";

const source = readFileSync(new URL("./error.tsx", import.meta.url), "utf-8");

describe("/learn/[courseSlug] error boundary", () => {
  ...
  it("renders a safe, generic error state and never leaks the underlying error", () => {
    ...
    const html = renderToStaticMarkup(
      createElement(LearnCourseError, { error: sensitiveError, reset: () => {} }),
    );
