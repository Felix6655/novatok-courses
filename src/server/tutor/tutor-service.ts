...
import {
  getPinnedLessonContext,
  getRelevantLessons,
} from "@/server/tutor/content-retrieval";
import {
...
export async function getTutorAnswer(
  ...
  const localized = request.locale
    ? await import("@/server/localized-content")
    : null;
  const course = request.locale
    ? ((await localized!.getLocalizedCourseBySlug(
        request.courseSlug,
        request.locale,
      )) ?? canonicalCourse)
    : canonicalCourse;
  ...
  const syllabus = request.locale
    ? await localized!.getLocalizedCourseContent(course.id, request.locale)
    : await getCourseModulesWithLessons(course.id);
  ...
  if (request.lessonSlug) {
    ...
    if (!pinned) {
      throw new TutorLessonNotFoundError(
        request.courseSlug,
        request.lessonSlug,
      );
    }
  ...
  if (request.locale) {
    ...
    candidateLessons = candidateLessons.map(
      (lesson) => localizedBySlug.get(lesson.slug) ?? lesson,
    );
  }
